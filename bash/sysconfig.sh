...
#creating a date variable to house a timestamp when error occurs which is added to error log entry
date=$(date "+%D %T")

function error-message {
	echo "User tried using an invalid argument ($1) with sysconfig.sh" | cat>> ~/errors.log
	exit 1
}
...
function error-exit {
	echo "Please rerun script with a valid option or use -h to see a list of valid options."
	echo "$date: User $USER attempted to use $1 as an argument for sysconfig.sh" | cat>> ~/errors.log
	exit 1
}

#This function displays help information if the user asks for it on the command line or gives us a bad command line
function displayhelp {
	echo "Please use one of the following options with this script: --host, --domain, --ipconfig, --os, --cpu, --memory, --disk, --printer"
	exit 0
}
...
function cleanup {
	 sudo rm /tmp/sysinfo.*
	 sudo rm /tmp/memoryinfo.* 
	 sudo rm /tmp/businfo.*
	 sudo rm /tmp/cpuinfo.*
	 sudo rm /tmp/sysreport.*
	 echo  "Cleaning up temp files"
	 exit 1
}


#Looks for ctrl-C signal and initiates cleanup function right away when it is caught
trap cleanup SIGINT
